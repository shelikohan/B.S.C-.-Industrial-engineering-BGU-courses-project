...
check_kziza <- function(quality_kziza) { #quality check for kziza (בדיקת איכות של הקציצה)
   if (quality_kziza == 0) { #reallity - good
      quality_kziza_result = rdiscrete(1,c(0.9,0.1), c(0,1)) #identify mistake if good (עובד איי מזהה קציצה תקינה בטעות כפגומה בהסתברות 0.15)
   }else { #reallity - defect
      quality_kziza_result = rdiscrete(1,c(0.8,0.2), c(1,0)) #identify mistake if damaged (עובד איי מזהה קציצה פגומה בטעות כתקינה בהסתברות 0.3)
   }
...
initial_counter <- 0
initial_approve <- 1 
initial_outcome <-  6*50*16 #global salaries
initial_income <-  0
...
maintenance_AD <-  trajectory("maintenance_AD") %>% #תקלה במכונות A-D
   set_attribute(key="defect",value= function() rdiscrete(1,c(0.75,0.25), 1:2) ,global=FALSE) %>% #station that is defects
   join(maintenance) 


maintenance_EJ <-  trajectory("maintenance_EJ") %>% #תקלה במכונות E-J
   set_attribute(key="defect",value= function() rdiscrete(1,c(1/6,3/6,2/6), 2:4) ,global=FALSE) %>% #station that is defects
   join(maintenance)
...
add_service_stations <-  trajectory("add_service_stations")%>%
   renege_in(600)  %>%
   addService("station1",function() rnorm(1, 8,sqrt(2.01))) %>% 
   addService("station2",function() rnorm(1, 6,sqrt(0.125))) %>% 
   addService("station3",function() rnorm(1, 9,sqrt(2.8325))) %>% 
   addService("station4",function() rnorm(1, 8,0.5)) %>% 
   renege_abort() 
...
mamaTov_factory%>%
   ...
   add_generator("defects AD",maintenance_AD , function() rnorm(1, 103.7368*60 ,5.950731*60), mon=2 ) %>%
   add_generator("defects EJ", maintenance_EJ, function() rexp(1, 0.006772104/60), mon=2 )
...
#-----------------------------------------------analysis_existing_situation-----------------------------------------------------------------------------------------------------------------------
 ...
 n <- 30
alternative2 <-mclapply(1:n, function(i) {
   set.seed(((i+98)^2)*3-7)
   reset(mamaTov_factory)%>%run(until=simulationTime)%>%
     wrap()
 })

 arrivalsData <- get_mon_arrivals(alternative2)
 attributesData <- get_mon_attributes(alternative2)
 service_data <-  get_mon_arrivals(alternative2, per_resource = TRUE)
 resourceData <-  get_mon_resources(alternative2)

 
 #------------------------------------------------------------avrage damaged kzizas in argaz -only accepted------------------------------------------------------------------------------------------
 ...
 #--------------------------------------------------workers in 4 stations utility-------------------------------------------------------------------------------
 
 workers_utility <- sqldf ( "
                            select replication, (sum(activity_time)+2*20*60*4)/ (4*16*60*60) as utility 
                            from service_data
