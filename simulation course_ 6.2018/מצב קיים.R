...
check_kziza <- function(quality_kziza) { #quality check for kziza (בדיקת איכות של הקציצה)
   if (quality_kziza == 0) { #reallity - good
      quality_kziza_result = rdiscrete(1,c(0.85,0.15), c(0,1)) #identify mistake if good (עובד איי מזהה קציצה תקינה בטעות כפגומה בהסתברות 0.15)
   }else { #reallity - defect
      quality_kziza_result = rdiscrete(1,c(0.7,0.3), c(1,0)) #identify mistake if damaged (עובד איי מזהה קציצה פגומה בטעות כתקינה בהסתברות 0.3)
   }
...
production_line<- trajectory("production_line")%>% #kziza traj
   ...
   set_attribute(key="quality", value=function() rdiscrete(1,c(0.85,0.15),c(0,1)), global = FALSE)  %>% #real quality- 0=good , 1=defect
...
#-----------------------------------------------analysis_existing_situation-----------------------------------------------------------------------------------------------------------------------
 ...
 n <- 34
 existing_situation<-mclapply(1:n, function(i) {
   set.seed(((i+100)^2)*3-7)
   reset(mamaTov_factory)%>%run(until=simulationTime)%>%
     wrap()
 })

 arrivalsData <- get_mon_arrivals(existing_situation)
 arrivalsData$'system time' = arrivalsData$end_time- arrivalsData$start_time
 attributesData <- get_mon_attributes(existing_situation)
 service_data <-  get_mon_arrivals(existing_situation, per_resource = TRUE)
 resourceData <-  get_mon_resources(existing_situation)


 #------------------------------------------------------------avrerage damaged kzizas in argaz -only accepted------------------------------------------------------------------------------------------
